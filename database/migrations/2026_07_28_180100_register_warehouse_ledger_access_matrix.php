...
return new class extends Migration
{
    ...
    private const MENUS = [
        ['code' => 'warehouse-ledger-history', 'name' => 'Stock History', 'path' => '/warehouse/ledger/history', 'sort_order' => 240, 'permission' => 'warehouse.ledger.history'],
        ['code' => 'warehouse-ledger-adjustments', 'name' => 'Stock Adjustment', 'path' => '/warehouse/ledger/adjustments', 'sort_order' => 250, 'permission' => 'warehouse.ledger.adjustment'],
        ['code' => 'warehouse-ledger-reconciliation', 'name' => 'Valuation & Reconciliation', 'path' => '/warehouse/ledger/reconciliation', 'sort_order' => 260, 'permission' => 'warehouse.ledger.reconciliation'],
    ];
    ...
    public function up(): void
    {
        ...
        foreach (self::MENUS as $menu) {
            ...
            $this->seedMatrixRows($menuId, $menu['code'], $now);
        }

        if (Schema::hasTable('permissions')) {
            foreach ([
                'warehouse.ledger.adjustment.reverse',
                'warehouse.ledger.negative_override',
                'warehouse.ledger.reconciliation.repair',
                'warehouse.ledger.reconciliation.export',
            ] as $permission) {
    ...
    private function seedMatrixRows(string $menuId, string $menuCode, $now): void
    {
        ...
        foreach ($roles as $role) {
            ...
            foreach (array_merge([null], $levels) as $levelId) {
                ...
                $canDelete = $enabled && ($menuCode !== 'warehouse-ledger-adjustments' || $roleCode === 'ADMIN');
