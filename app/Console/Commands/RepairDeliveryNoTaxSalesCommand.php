...
class RepairDeliveryNoTaxSalesCommand extends Command
{
    ...
    protected $description = 'Audit and optionally repair DELIVERY sales so tax is always zero for reprint and reports.';
    ...
    public function handle(): int
    {
        ...
        $query = Sale::query()
            ...
            ->where(function (Builder $builder) {
                $builder->where('tax_percent_snapshot', '>', 0)
                    ->orWhere('tax_total', '>', 0)
    ...
    private function computeCorrectedTotals(Sale $sale): array
    {
        ...
        $needsUpdate =
            (int) ($sale->tax_percent_snapshot ?? 0) !== $taxPercent
            || (int) ($sale->tax_total ?? 0) !== $taxTotal
            || (int) ($sale->rounding_total ?? 0) !== $roundingTotal
            || (int) ($sale->grand_total ?? 0) !== $grandTotal
            || (int) ($sale->paid_total ?? 0) !== $paidTotal
            || (int) ($sale->change_total ?? 0) !== $changeTotal
            || !is_null($sale->tax_id);
