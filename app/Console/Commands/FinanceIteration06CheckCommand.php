...
class FinanceIteration06CheckCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        if(in_array(DB::getDriverName(), ['mysql','mariadb'], true)){
            try{
                $exists=DB::select("SELECT TRIGGER_NAME FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA = DATABASE() AND TRIGGER_NAME = 'finance_iter06_guard_recon_reversal'");
                $trigger=$exists?'DB_TRIGGER + APP_GUARD':'APP_GUARD';
            }catch(\Throwable $e){
                $trigger='APP_GUARD';
            }
        }
