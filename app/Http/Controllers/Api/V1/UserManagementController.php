...
class UserManagementController extends Controller
{
    ...
    public function overview(Request $request)
    {
        ...
        // ERP POS FINAL I07 — server-side User Management filters.
        // Keep parameter names stable for the existing I07 frontend contract.
        $assignment = trim((string) $request->string('assignment', ''));
        ...
        if ($assignment !== '') {
            $usersQuery->whereHas('employee.assignment', function ($assignmentQuery) use ($assignment) {
                $assignmentQuery->where('role_title', $assignment);
            });
        }
    ...
    private function buildMasters(): array
    {
        ...
        if (
            ...
            && Schema::hasColumn('employees', 'assignment_id')
            && Schema::hasColumn('assignments', 'role_title')
        ) {
            $assignments = DB::table('employees as e')
                ->join('assignments as a', 'a.id', '=', 'e.assignment_id')
                ->whereNotNull('e.user_id')
                ->whereNotNull('a.role_title')
                ->whereRaw("trim(coalesce(a.role_title, '')) <> ''")
                ->selectRaw('trim(a.role_title) as role_title')
                ->distinct()
                ->orderBy('role_title')
                ->pluck('role_title')
        ...
        return [
            ...
            'menus' => $menus,
            'outlets' => $outlets,
            // I07 canonical current-assignment dropdown values.
